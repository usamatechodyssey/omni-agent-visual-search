...
def _normalize_details(details: Any) -> Optional[dict]:
    """
    DB ke 'details' column mein kabhi kabhi list (e.g. []) aajati hai jab
    dict expected hai. Pydantic ko hamesha dict ya None dete hain taake
    validation crash na ho.
    """
    if isinstance(details, dict):
        return details
    if not details:  # None ya empty list []
        return None
    # Agar list mein waqai kuch data pada hai (unexpected case), usay zaya
    # na karke wrap kardo taake debug karna aasan rahe.
    return {"raw": details}
...
async def get_all_jobs(
    ...
    return [
        JobStatusResponse(
            ...
            total_items=job.total_items,
            error_message=job.error_message,
            details=_normalize_details(job.details),
        )
        for job in jobs
    ]
